## 设计模式的六大法则
- 里氏代换 子类可以完全有父类替代，要求子类实现父类的所有方法，且不修改父类的实现  正方形的例子
- 最小知道法则 应该只和朋友通信（成员变量、方法、参数）
- 依赖倒置 依赖于抽象而不是实现
- 接口隔离 接口要最小，可以拆分出
- 单一职责  专注于单一的功能，避免臃肿 举例子 内容发布系统，有一个 Article类处理文章本身的数据和操作，持久化、内容展示、格式转换不应该出现
- 开闭原则 对扩展开放，对修改关闭， 反例ifelse——正例抽象出接口，每一次继承拓展

## 重载和重写的区别

## 抽象类和普通类有社么区别
抽象类abstract修饰符，可以不实现方法，代表抽象，共性，无法被final修饰，无法直接实例化  
## 接口和抽象类的区别
共性：抽象、共性  接口偏向于行为，抽象类侧重类的继承关系  多继承 关键字  
抽象类的成员变量可以是静态、实例常量，接口只能是静态常量    

## 静态内部类和非静态内部类的区别

## 非静态内部类为什么可以访问类的方法

## final的作用
- 类
- 方法
- 属性

## static的作用
- 变量
- 方法
- 代码块
- 内部类

## 深拷贝和浅拷贝的区别

## 深拷贝的实现方式
- 实现clone方法
- 序列化反序列化

## 泛型是社么

## 泛型擦除是社么，历史背景

## BIO、NIO、AIO
- 同步阻塞、非同步阻塞、异步io
- 等待数据到来，wait、notify
- selector监听channel，将数据从缓冲区copy
- 利用操作系统完成io，最后通知java

## Reactor && Proactor
- Reactor 操作系统完成通知，应用程序完成读写；Proactor 应用程序异步发起io请求，交给操作系统完成以后通知应用程序
- window选择r——api简单、企业使用；linux使用p———Unix提供工具开放；Redis使用r———io不是瓶颈，内存和CPU才是，单线程，linux

## 排序的方式
- 实现Comparactor接口CompareTo方法，配合Collections.sort
- 创建自定义排序类，Compartor类，lamdba简化，sort链式

## native、JNI、JIT、Java、OS线程、JVM
- native本地方法，C/C++编写的代码，JNI进行管理和调用
- JIT是热点优化技术，对于频繁使用的代码，JVM会进行优化具体表现是机器码
- JVM创建的线程本质上就是操作系统线程，JVM也是一个进程，内存管理、io调用C

## Java的内存模型介绍一下

## 并发核心问题
### 有序性
核心问题：
解决方式：
底层原理：
## 三大策略
### happens-before

### 主存/工作内存 

### 关键字


## Java多线程是什么，需要注意什么

- 安全
- 通信
- 成本

## Java的多线程和操作系统的多线程一样吗

## 如何保证数据一致性呢

## 线程创建的方式？
- Thread 继承
- Runnable接口
- Callable接口
- 线程池

Thread.start 做了什么：
1. 调用JNI的start0
2. 操作系统pthread_creat
3. 分配栈空间、程序计数器
4. 放入就绪队列
5. 等待CPU时间
6. 调用run方法

Runnable优势原理  
Callable优势原理  
  
## 如何启动一个线程

## 如何停止一个线程

## interrupt的工作机制

## Java的线程的状态
 
## sleep和wait介绍

## blocked和waiting的区别
- 触发条件
- 唤醒机制
- 原理不同

## wait如何恢复到running

## notify和notifyAll的区别

## notify会选择哪一个线程

## park/unpark介绍

## 对象头介绍

## ObjectWaiter对象介绍

## ObjectWaiter工作流程

## 虚假唤醒

## park/unpark

## Lock/signal/await

## 惊群效应

## 不同的线程如何通信

## Go的协程和Java的线程有什么区别

## juc下有什么功能
- 线程池相关
- 并发集合
- 关键字
- 同步工具
- 原子类

## 如何保证并发安全

## 有哪些锁

## CountDownLatch

## syncronized和reentranklock的区别

## 如何理解可重入锁

## syncronized的锁升级讲解一下

## syncronized的可重入是如何实现的

## AQS是什么

## CAS和AQS的区别

## 如何使用AQS实现一个可重入的公平锁

## ThreadLocal的作用，原理，key/value，会有什么问题，如何解决的

## ThreadLocal的数据结构

## ThreadLocal的方法，扩容，哈希法

## 严格雪崩？斐波那契散列？除法散列

## 一致性哈希/带虚拟节点的一致性哈希

## 悲观锁和乐观锁的区别

## 如何实现一个乐观锁

##  CAS有什么优缺点

## 为什么不能全部锁都使用CAS

## volatile的原理

## 屏障的类型
- 写写
- 写/读写
- 读写

## volatile和sychronized的区别

## 公平锁和非公平锁

## 为什么非公平锁的吞吐量会比公平锁大

## synchronized是公平锁码

## 介绍一下死锁问题，如何避免

## 死锁的避免、检测、恢复
JDK 自带命令 jstack：
使用 jps或 ps命令找到Java进程的PID。
运行 jstack <pid>。
在输出信息底部，如果能找到 Found one Java-level deadlock:字样，下面会清晰地列出哪些线程在互相等待，以及它们持有的锁和等待的锁 。
JConsole 或 JVisualVM：这些图形化工具可以更直观地查看线程状态和死锁信息。
## 介绍一下线程池

## 介绍一下线程池的参数

## 介绍一下线程池的原理

## 线程池满了以后的拒接策略

## 线程池的设置实战
### 电商
### 后台任务
### HTTP
io密集，依赖下游，redis

## 线程池
通过worker、aqs、阻塞队列实现调度管理，典型的生产消费者模型，将任务逻辑和执行单元分离，worker继承aqs，实现了runnable，本身是任务也是锁

## 核心线程数可以设置为0吗

## 线程池的种类
- fixThreadPool
- cacheThreadPool
- 定时
- syncnizioned

## 线程池对比普通线程有什么优点

## 线程池中的设计模式

## 线程池的shutdonw和shutdownNow

## 提交给线程池的任务可以撤销吗

## 多线程打印奇偶数，怎么控制打印的顺序单例模型既然已经用了
## synchronized，为什么还要在加volatile？
## 3个线程并发执行，1个线程等待这三个线程全部执行完在执行，怎么实现？
## 假设两个线程并发读写同一个整型变量，初始值为零，每个线程加 50 次，结果可能是什么？
